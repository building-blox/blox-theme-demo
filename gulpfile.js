...
gulp.task('watch-templates', function (done) {
  gulp.watch('src/templates/**/**/*.njk', gulp.series('blox:build')).on('change', stackReload)
  var timer = null
...
//watch scripts
  ...
  gulp.watch([
    'src/assets/js/**/*.js', 
    'src/templates/**/*.js'
  ], gulp.series('blox:build')).on('change', stackReload)
  var timer = null
  function stackReload (file) {
    if (timer) clearTimeout(timer)
    if (!gulp.isRunning) {
  ...
  gulp.task('watch-sass', function (done) {
    gulp.watch([
      ...
      'src/templates/**/*.scss'
    ], gulp.series('sass', 'blox:build')).on('change', stackReload)
    var timer = null
    function stackReload (file) {
      if (timer) clearTimeout(timer)
      if (!gulp.isRunning) {
  ...
  gulp.task('watch-data', function (done) {
    gulp.watch('src/data/*.json', gulp.series('blox:build')).on('change', stackReload)
    var timer = null
    function stackReload (file) {
      if (timer) clearTimeout(timer)
      if (!gulp.isRunning) {
...
gulp.task(
  'default',
  gulp.series('clean', gulp.parallel('watch-templates', 'watch-scripts', 'watch-sass', 'watch-data'), 'blox:dev', 'build-custom', 'browser-sync')
);